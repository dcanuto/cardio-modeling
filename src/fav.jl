function fav(x::Vector{Float64},system::CVSystem,n::Int64,state::String)
    ...
    if state == "opening"
        f2 = (x[3] - system.solverparams.h*((zs^-1-x[3])*system.heart.av.Kvo*
        ...
        f3 = (x[3]^3*(system.heart.lv.V[n+1]/Vs-x[2])*ts/system.solverparams.h-
            system.heart.av.Aann*ts/(system.solverparams.rho*system.heart.av.leff*Vs)*
            ((zs*x[3]^3-system.heart.av.zeta[n+1]*(x[3])^2)/(system.heart.av.Kvo*zs^2*
            (zs^-1-x[3]))-zs^-3*system.solverparams.rho*Vs^2/(2*system.heart.av.Aann^2*
            system.solverparams.h)*(system.heart.lv.V[n+1]/Vs-x[2])*
            abs(system.heart.lv.V[n+1]/Vs-x[2])-x[3]^2*system.heart.av.Ks*
            system.heart.lv.E[n+2]*Vs^2*(x[2]-system.heart.lv.V0/Vs)*
            (system.heart.lv.V[n+1]/Vs-x[2]))-
            ts/Vs*x[3]^3*system.branches.Q[1][n+1,1]);
    else
        ...
        f3 = (x[3]^3*(system.heart.lv.V[n+1]/Vs-x[2])*ts/system.solverparams.h-
            system.heart.av.Aann*ts/(system.solverparams.rho*system.heart.av.leff*Vs)*
            ((zs*x[3]^2-system.heart.av.zeta[n+1]*x[3])/(system.heart.av.Kvc*zs^2)
            -zs^-3*system.solverparams.rho*Vs^2/(2*system.heart.av.Aann^2*
            system.solverparams.h)*(system.heart.lv.V[n+1]/Vs-x[2])*
            abs(system.heart.lv.V[n+1]/Vs-x[2])-x[3]^2*system.heart.av.Ks*
            system.heart.lv.E[n+2]*Vs^2/zs*(x[2]-system.heart.lv.V0/Vs)*
            (system.heart.lv.V[n+1]/Vs-x[2]))-
            ts/Vs*x[3]^3*system.branches.Q[1][n+1,1]);
    end
    # f3 = (x[3]^3*(system.heart.lv.V[n+1]/vs-x[2])/system.solverparams.h*ts-
    #     ts*system.solverparams.h*system.heart.av.Aann/(system.solverparams.rho*
    #     system.heart.av.leff*Vs)*(x[3]^2/zs*(system.heart.lv.E[n+2]*Vs*
    ...
    #     system.branches.A0[1][end]))-system.solverparams.rho*Vs^2/
    #     (2*system.heart.av.Aann^2*zs^3*system.solverparams.h^2)*
    #     (system.heart.lv.V[n+1]/Vs-x[2])*abs(system.heart.lv.V[n+1]-x[2])-
    #     system.heart.av.Ks*system.heart.lv.E[n+2]*Vs^2/
    #     (zs*system.solverparams.h)*x[3]^2*(x[2]-system.heart.lv.V0/Vs)*
    #     (system.heart.lv.V[n+1]/Vs-x[2]))-ts/Vs*x[3]^3*system.branches.Q[1][n+1,1]);
    f = [f1,f2,f3];
end
